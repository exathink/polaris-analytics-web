export const PROJECTS_ALIGNMENT_TRENDS_WIDGETS = "projects.alignment-trends-widgets";
export const VALUE_STREAM_CONFIG = "projects.value_stream_config";
export const VIDEO_GUIDANCE = 'system.video_guidance';
export const VALUE_STREAM_VALUE_DASHBOARD = 'projects.value-dashboard';
export const TRELLO_CONNECTOR = 'connectors.trello';
export const SYSTEM_TEAMS = 'system.teams';
export const AZURE_DEVOPS_CONNECTOR = 'connectors.azure-devops'
export const UI_NEW_CARD_DESIGN = 'ui.new-card-design'
export const DEMO_MODE = 'system.demo_mode'
export const FLOW_EFFICIENCY_DASHBOARDS = 'projects.flow_efficiency_dashboards'


