...
describe("Flow Efficiency Measurement", () => {
  ...
  test("when there are multiple workItems, with phases deliver", () => {
    const workItems = [
      {
        name: "Cycle Time Card on Flow Summary not updating when specs/all is toggled. ",
        key: "335acba7-2b8c-433b-abc3-df3cea1e15e4",
    ...
    expect(deliveryCycleDurationsByState).not.toHaveProperty("In Progress");
    expect(deliveryCycleDurationsByState).not.toHaveProperty("CODE REVIEW");
  });
});
