  ...
  return (
    <Dashboard dashboard={`${dashboard_id}`} className="tw-bg-ghostwhite tw-grid tw-grid-cols-6 tw-grid-rows-[7%_auto_auto_auto_5%_50%] tw-gap-2 tw-p-2" gridLayout={true}>
      <div className="tw-col-start-1 tw-col-span-2 tw-row-start-1 tw-text-2xl tw-text-gray-300">
        <div className="tw-flex tw-justify-start">Flow Volume, {limitToSpecsOnly ? AppTerms.specs.display : `All ${AppTerms.cards.display}`}</div>
      ...
      <DashboardRow>
        <DashboardWidget
          name="volume-trends"
          className={cn((selectedMetric === "workItemsWithCommits" || selectedMetric === "workItemsInScope") ? "" : "!tw-hidden", "tw-col-start-1 tw-row-start-5 tw-row-span-2 tw-col-span-6")}
          render={({view}) => (
        ...
        <DashboardWidget
          ...
          className={cn(
            ...
            "tw-col-start-1 tw-row-start-5 tw-row-span-2 tw-col-span-6"
          )}
          render={({view}) => (
