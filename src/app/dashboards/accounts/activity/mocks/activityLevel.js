...
if (process.env.NODE_ENV === 'development') {
  ...
  polarisChartsService.registerMockDataSource(DataSources.activity_level_for_account_by_organization, {
    fetchData: (dataSource, params) => {
      console.log("fetching stub data..");
      const data = [];
      const items = params.items || 2000;
      const now = moment();
      for (let i = 0; i < 0.7*items; i++) {
        ...
        data.push({
          organization_key: `${chance.integer() + i}`,
          organization: chance.animal() + `(${i})`,
      ...
      for (let i = 0; i < 0.20*items; i++) {
        const latest_commit = moment(now).subtract(chance.integer({min:0, max:90}), 'days');
      ...
      for (let i = 0; i < 0.1*items; i++) {
        const latest_commit = moment(now).subtract(chance.integer({min:0, max:40}), 'days');
        const earliest_commit = moment(latest_commit).subtract(chance.integer({min:1, max:120}), 'days');
        data.push({
          organization_key: `${chance.integer() + i}`,
          organization: chance.animal() + `(${i})`,
