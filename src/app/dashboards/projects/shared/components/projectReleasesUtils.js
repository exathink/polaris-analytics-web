...
const defaultItem = {key: "all", name: "Releases"};
...
export function useQueryParamSync({uniqueItems, valueIndex, updateFromQueryParam}) {
  ...
  const valueStreamKey = queryParams.get("release");
  ...
  React.useEffect(() => {
    ...
    if (valueIndex === 0) {
      ...
      history.push({search: `?release=${uniqueItems[valueIndex].key}`, state: uniqueItems[valueIndex]});
    }
...
export function ProjectReleasesWidget({context}) {
  const projectKey = context.getInstanceKey("project");
  const result = useQueryReleases({projectKey, releasesWindow: 90});
